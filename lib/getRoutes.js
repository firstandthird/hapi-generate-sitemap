  ...
  const parseRoute = async (table) => {
    ...
    if (table.path.indexOf('{') !== -1) {
      if (options.dynamicRoutes) {
        ...
        if (addRoutes) {
          addRoutes.forEach(r => {
            const obj = typeof r === 'object' ? r : { path: r, section: 'none' };
    ...
    if (!options.excludeUrls.includes(table.path)) {
      const pluginSettings = table.settings.plugins['hapi-generate-sitemap'];
      results.push({
        path: table.path,
        section: pluginSettings ? pluginSettings.section : 'none',
